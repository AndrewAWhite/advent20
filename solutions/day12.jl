...
function change_course(bearing, instruction)
    change = instruction[2] / 90
    if instruction[1] == 'L'
        change = 4-change
    end
    compass = ['E', 'S', 'W', 'N']
    i = (findfirst(c->c==bearing, compass) + change)%4
    if i == 0
        i = 4
    end
    i = convert(Int64, i)
...
function read_instructions()
    return readlines("data/input_d12q1.txt") .|> x -> [x[1], parse(Int64, x[2:end])]
end
...
function q1()
    ...
    for instruction in instructions
        mov = nothing
        if instruction[1] in ['L', 'R']
            bearing = change_course(bearing, instruction)
        elseif instruction[1] == 'F'
            instruction = [bearing, instruction[2]]
            mov = instruction
        end
        if instruction[1] in ['S', 'W']
            instruction[2] *= -1
            instruction[1] = instruction[1] == 'S' ? 'N' : 'E'
            mov = instruction
        else
            mov = instruction
        end
...
function rotate_waypoint(waypoint, instruction)
    rotate = instruction[2]/90
    if instruction[1] == 'L'
        rotate = 4-rotate
    end
    rotate = convert(Int64, rotate%4)
    if rotate == 0
        return waypoint
    elseif rotate == 1
...
function q2()
    ...
    for instruction in instructions
        if instruction[1] in ['L', 'R']
            ...
            continue
        elseif instruction[1] == 'F'
            pos[1] += instruction[2] * waypoint[1]
            pos[2] += instruction[2] * waypoint[2]
            continue
        end
        if instruction[1] in ['S', 'W']
            instruction[2] *= -1
