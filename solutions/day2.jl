...
function get_line_spec(line)
    line_split = split(line, ": ")
    spec_split = split(line_split[1], " ")
    range = split(spec_split[1], "-")
    char = spec_split[2]
    return parse(Int64, range[1]), parse(Int64, range[2]), char, line_split[2]
end

function q1()
    input = read("../data/input_d2q1.txt", String)
    valid_count = 0
    for line in split(input, "\n")
        spec = get_line_spec(line)
        count = 0
        for char in spec[4]
            if char != spec[3][1]
                continue
...
function q2()
    ...
    for line in split(input, "\n")
        spec = get_line_spec(line)
        p1 = spec[4][spec[1]][1] == spec[3][1]
        p2 = spec[4][spec[2]][1] == spec[3][1]
        if xor(p1, p2)
            valid_count += 1
        end
    end
