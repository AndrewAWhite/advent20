...
function split_input(input)
    # take in source data, return one passport per line
    single_line = replace(input, r"([^^])\n"m => s"\1 ") # replace any newline not on a blank line with a space
    return split(single_line, "\n")
end

function convert_dict(single_line)
    # take in passport string, output dictionary
    line_split = split(single_line, " ")
    return Dict(split(ls,":")[1] => split(ls,":")[2] for ls in line_split if ls != "")
end

function iter_lines(input)
    # iterate over the source data, yielding a dictionary representation for each passport
    Channel() do channel
...
function iter_valid_lines(input)
    # iterate over source data, yielding only dictionaries for which all required keys are present
    required_keys = [
        "byr" ,
        "iyr",
        "eyr",
    ...
    Channel() do channel
        for d in iter_lines(input)
            if all(k in keys(d) for k in required_keys)
                put!(channel, d)
            end
        end
    end
...
function q1()
    input = read("../data/input_d4q1.txt", String)
    count = 0
    for line in iter_valid_lines(input)
        count += 1
    end
    return count
end
...
function check_int_rules(d, rules)
    # take in passport dictionary and list of integer rules
    # output true if all rules pass, false otherwise
    # rules is a list containing rules of form ((passport key), (min_val, max_val))
    for (k, v) in rules
        to_check = d[k]
        val = tryparse(Int64, to_check)
        if isnothing(val)
            return false
        end
        if !(v[1] <= val <= v[2])
...
function check_hgt_rule(d)
    # take in passport dictionary, return true if passes height rule,
    # false otherwise
    m = match(r"([0-9]+)(cm|in)", d["hgt"])
    if isnothing(m) || m.match != d["hgt"]
        return false
    end
    h = parse(Int64, m.captures[1])
    if m.captures[2] == "cm" && 150 <= h <= 193
        return true
    elseif m.captures[2] == "in" && 59 <= h <= 76
        return true
    end
    return false
end

function check_pattern_rules(d, rules)
    # take in passport dictionary and regex rules
    # output true if all rules pass, false otherwise
    # rules is a list containing rules of form ((passport key), (regex pattern))
    for rule in rules
        m = match(rule[2], d[rule[1]])
        if isnothing(m) || m.match != d[rule[1]]
            return false
        end
    end
    return true
...
function check_valid(input)
    ...
    valid_count = 0
    for d in iter_valid_lines(input)
        if !(check_pattern_rules(d, pattern_rules))
            continue
        end
        if !(check_int_rules(d, int_rules))
            continue
        end
        if !(check_hgt_rule(d))
            continue
